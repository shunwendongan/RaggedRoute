 ...
 public:
  ...
  void enqueue(MeasurementLevel level, cudaStream_t stream) override {
    ...
    if (is_optimized_histogram_variant(variant_name_)) {
      ...
      if (implementation == ops::kHistogramCandidateV2Implementation) {
        if (ops::histogram_optimized_requires_external_reset(
                implementation, static_cast<int>(ids_host_.size()), experts_)) {
          cuda_check(cudaMemsetAsync(counts_.data(), 0, counts_.bytes(), stream),
                     "reset benchmark-only Histogram v2 counts");
        }
        cuda_check(ops::launch_histogram_optimized(
                       ids_.data(), counts_.data(), static_cast<int>(ids_host_.size()), experts_,
                       implementation, stream),
                   "launch benchmark-only Histogram v2 operator path");
        return;
      }
      HistogramArgs args;
