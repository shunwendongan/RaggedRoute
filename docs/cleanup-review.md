...
最新 live 复核：2026-08-30（Asia/Shanghai），`main@2366acec5d2ab6552e6112e5019ac948a51fbf34`
治理结果：`codex/repo-governance-performance-docs` 已通过 [PR #38](https://github.com/shunwendongan/RaggedRoute/pull/38) 合入；套餐完成审计又通过 [PR #39](https://github.com/shunwendongan/RaggedRoute/pull/39) 合入，两个远端任务分支均已自动删除。

执行结果：初轮 29 个 merged-PR 远端分支已按 live SHA 和 `merged_at` 复核后删除；后续 PR #36/#37/#38/#39 的任务分支由 `delete_branch_on_merge=true` 自动清理。2026-08-30 在 PR #39 合并后再次读取 `git ls-remote`、PR 状态和仓库设置，远端只保留 `main` 与无关联 PR 的 `codex/grouped-gemm-sm86-v2`。4 个 evidence tag 不变。本地分支、5 个 worktree、1 个 stash 和未跟踪实验文件均未删除。
