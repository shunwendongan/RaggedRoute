...
- 解释：减少 M tile 的确增加 CTA 并在 T1024 缓解 v6 的粗粒度损失，但每个额外 M tile 都重复加载相同的 `Kx128` weight tile；随着 token rows 或 N 增大，额外 weight traffic 与更差复用压过 underfill 收益。下一轮若继续隔离几何，优先测试 `32x64`：通过 N 方向增加 CTA，重复的是更小的 A tile，而不是更大的 weight tile。
