...
__device__ __forceinline__ void stage_sync_wide(const float* x_permuted,
  ...
  for (int index = thread; index < kDepth * kColumns; index += kThreads) {
    const int inner = index / kColumns;
    const int local_column = index - inner * kColumns;
    weight_tile[inner * kColumns + local_column] =
        expert_weights[(static_cast<std::size_t>(expert) * hidden + k_base + inner) * output +
                       tile_column * kColumns + local_column];
  }
}
...
__device__ __forceinline__ void stage_async_wide(const float* x_permuted,
  ...
  constexpr int kWeightVectors = kDepth * kColumns / 4;
  for (int vector = thread; vector < kWeightVectors; vector += kThreads) {
    const int element = vector * 4;
    const int inner = element / kColumns;
    const int local_column = element - inner * kColumns;
    const float* source = expert_weights +
                          (static_cast<std::size_t>(expert) * hidden + k_base + inner) * output +
                          tile_column * kColumns + local_column;
    copy_float4_async_wide(weight_tile + inner * kColumns + local_column, source);
  }
  commit_async_wide();
}
...
__device__ __forceinline__ void accumulate_wide(
    ...
    float (&accumulator)[WideConfig<BlockRows>::kRowsPerThread][kColumnsPerThread]) {
#pragma unroll
  for (int inner = 0; inner < kDepth; ++inner) {
    float x_values[WideConfig<BlockRows>::kRowsPerThread];
    float weight_values[kColumnsPerThread];
#pragma unroll
    for (int row = 0; row < WideConfig<BlockRows>::kRowsPerThread; ++row) {
      x_values[row] = x_tile[(local_row + row) * kAStride + inner];
    }
#pragma unroll
    for (int column = 0; column < kColumnsPerThread; ++column) {
      weight_values[column] = weight_tile[inner * kColumns + local_column + column];
    }
#pragma unroll
    for (int row = 0; row < WideConfig<BlockRows>::kRowsPerThread; ++row) {
#pragma unroll
...
__device__ __forceinline__ void store_wide(
    ...
    const float (&accumulator)[WideConfig<BlockRows>::kRowsPerThread][kColumnsPerThread]) {
#pragma unroll
  for (int row = 0; row < WideConfig<BlockRows>::kRowsPerThread; ++row) {
...
__global__ __launch_bounds__(kThreads, 2) void grouped_gemm_sm86_balanced_wide_kernel(
  ...
  __align__(16) __shared__ float weight_tiles[2][kDepth][kColumns];

  const int warp = static_cast<int>(threadIdx.x) >> 5;
  const int lane = static_cast<int>(threadIdx.x) & 31;
  const int local_row = warp * WideConfig<BlockRows>::kRowsPerThread;
  const int local_column = lane * kColumnsPerThread;
  const bool full_rows = (tile_row + 1) * BlockRows <= rows;
  float accumulator[WideConfig<BlockRows>::kRowsPerThread][kColumnsPerThread] = {};

  if (!full_rows) {
    for (int k_base = 0; k_base < hidden; k_base += kDepth) {
      stage_sync_wide<BlockRows>(x_permuted, expert_weights, expert, begin, rows, hidden, output,
                                 tile_row, tile_column, k_base, &x_tiles[0][0][0],
                                 &weight_tiles[0][0][0]);
      __syncthreads();
      accumulate_wide<BlockRows>(&x_tiles[0][0][0], &weight_tiles[0][0][0], local_row, local_column,
                                 accumulator);
      __syncthreads();
    }
  } else if (hidden > 0) {
    stage_async_wide<BlockRows>(x_permuted, expert_weights, expert, begin, hidden, output, tile_row,
    ...
    for (int k_base = 0; k_base < hidden; k_base += kDepth) {
      ...
      if (next_k < hidden) {
        stage_async_wide<BlockRows>(x_permuted, expert_weights, expert, begin, hidden, output,
                                    tile_row, tile_column, next_k, &x_tiles[stage ^ 1][0][0],
                                    &weight_tiles[stage ^ 1][0][0]);
      }
      accumulate_wide<BlockRows>(&x_tiles[stage][0][0], &weight_tiles[stage][0][0], local_row,
  ...
  store_wide<BlockRows>(y_permuted, begin, rows, output, tile_row, tile_column, local_row,
                        local_column, accumulator);
}
...
cudaError_t launch_balanced_wide_kernel(const float* x_permuted, const float* expert_weights,
  ...
  const std::size_t grid_x = static_cast<std::size_t>(output) / kColumns;
  ...
  grouped_gemm_sm86_balanced_wide_kernel<BlockRows>
