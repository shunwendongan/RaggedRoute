...
Status launch_separate(const HistogramExclusiveScanArgs& args,
                       const RuntimeContext& context, KernelSelection histogram_kernel,
                       KernelSelection scan_kernel) noexcept {
  ...
  histogram_args.kernel = histogram_kernel;
...
Status histogram_exclusive_scan(const HistogramExclusiveScanArgs& args,
  ...
  if (decision.kernel.family == KernelFamily::kCudaNaive) {
    return launch_separate(args, context, {KernelFamily::kCudaNaive, 0},
                           {KernelFamily::kCudaNaive, 0});
  }
  ...
  // The research candidate is single-CTA by construction. Larger inputs use
  // the already promoted Histogram dispatcher followed by the Scan dispatcher.
  return launch_separate(args, context, {}, {});
}
